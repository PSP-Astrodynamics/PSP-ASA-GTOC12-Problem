...
scale = [0, 0, pi/180, pi/180, pi/180, pi/180];
offset = 2;
[v, i] = min(vecnorm(scale .* (y.data(ID1, offset + (1 : 6)) - y.data([1 : (ID1 - 1), (ID1 + 1) : end], offset + (1 : 6))), 2, 2))
ID2 = i;
...
terminal_bc = @(x, p, x_ref, p_ref) [x(1:6) - (x_cartesian_ast2(p_ref(1) + t0) + [eye(6), zeros([6, 1])] * f(p_ref(1), [x_cartesian_ast2(p_ref(1) + t0); 1e-5], zeros([3, 1]), p_ref) * (p(1) - p_ref(1))); 0];
...
if initial_guess == "straight line"
    ...
    nu_guess = interp1(tspan, [nu_ast1(t0), nu_ast2(tf_actual) + 2 * pi * floor(N_guess)]', t_k);
...
elseif initial_guess == "Lambert"
    ...
    [v1_best, v2_best, dV_best, ToF_best, N_best] = best_lambert(repmat(x_0(1:6), 1, numel(tofs)), x_f_tofs, tofs, [floor(N_guess), ceil(N_guess)], 6 / v_star, 0);
    N_best = N_best;
...
problem = DeterministicProblem(x_0, x_f, N, u_hold, 1, f, guess, convex_constraints, min_fuel_objective, scale = scale, initial_bc = initial_bc, terminal_bc = terminal_bc, integration_tolerance = 1e-12, discretization_method = "error", N_sub = 1);
...
scatter3(x_cartesian_ast1_plot(1, end), x_cartesian_ast1_plot(2, end), x_cartesian_ast1_plot(3, end), "green")
...
legend('Spacecraft', "", "Thrust", "Launch Velocity", 'Guess', "", sprintf("Asteroid %g", ID1), "", sprintf("Asteroid %g", ID2), "", "Start", "End", 'Location', 'northwest'); axis equal; grid on
...
nexttile
plot(t_cont_sol(1:end - (N - Nu)) * tf / year_to_sec * t_star, u_cont_sol(1:3,:), LineWidth=1); hold on
plot(t_cont_sol(1:end - (N - Nu)) * tf / year_to_sec * t_star, vecnorm(u_cont_sol(1:3,:)), LineWidth=1)
title("Control")
xlabel("Time")

nexttile
plot(t_cont_sol(1:end) * tf / year_to_sec * t_star, x_cont_sol(7, :))
title("Mass")
xlabel("Time")
