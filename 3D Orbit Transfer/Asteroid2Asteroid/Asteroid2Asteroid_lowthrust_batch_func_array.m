function [dataset, ptr_sols_all, converged_is_all] = Asteroid2Asteroid_lowthrust_batch_func_array(ID1, ID2, m0_frac, t0_yr, ToF_yr, paths, options)
arguments
    ID1
    ID2
    m0_frac
...
for i_ID1 = 1 : num_ID1_batch
    ...
    for i = 1 : num_filter_batch
        ...
        lambert_filter = find(dV_best_AA_batch < max_dV * options.lambert_multiplier);
        ...
        elseif options.surrogate == "Lambert"
    ...
    % 
    % rvec = x_1_AA_filtered(1:3, :);
    % vvec = v1_best_AA_filtered(1:3, :);
    % 
    % r = vecnorm(rvec);
    % v = vecnorm(vvec);
    % 
    ...
    problem = DeterministicProblem([x_1_AA_filtered(1:6, 1); m0(1)], x_2_AA_filtered(1:6, 1), N, u_hold, tf, f, guess_0, convex_constraints, min_fuel_objective, scale = scale, integration_tolerance = 1e-12, discretization_method = "errorRK4_kepler_fixedtf", N_sub = 1, Name = "Ast2Ast_fixed");
    ...
    parfor i = 1 : ceil(n_guesses / options.target_SCP_batch_size)
        ...
        for j = 1 : options.target_SCP_batch_size
            ...
            problem_new = problem;
            problem_new.x0 = [x_1_AA_filtered(1:6, index); m0_filtered(index)];
    ...
    valid_ID1_i = IDs_1_i(comb_i.IDs_1_i_batch(dV_filter(converged_is)));
    valid_ID2 = ID2(comb_i.IDs_2_i(dV_filter(converged_is)));
    ...
    %%
    % Ast1 -> Ast2
    x_1_kep = x_kep_ast1(:, comb_i.IDs_1_i_batch(dV_filter));
    x_2_kep = x_kep_ast2(:, comb_i.IDs_2_i(dV_filter));
    ...
    dataset.t0 = [dataset.t0, t0];%(comb_i.IDs_1_i_batch(dV_filter(converged_is)));
    dataset.m0 = [dataset.m0, m0(comb_i.IDs_1_i_batch(dV_filter(converged_is)))];
    dataset.mf = [dataset.mf, reshape(ptr_sols.x(7, end, converged_is), 1, [])];
    dataset.dV_max = [dataset.dV_max, dV_max(comb_i.IDs_1_i_batch(dV_filter(converged_is)))];
    ...
    dataset.ID1 = [dataset.ID1, ID1(comb_i.IDs_1_i_batch(dV_filter(converged_is)))];
    dataset.ID1_i = [dataset.ID1_i, valid_ID1_i];
    dataset.ID2 = [dataset.ID2, ID2(comb_i.IDs_2_i(dV_filter(converged_is)))];
    ...
    dataset.paths = [dataset.paths, [paths(:, valid_ID1_i); valid_ID2]];
    ...
    if options.plot
        ...
        for ic = 1 : min(20, numel(converged_is))
            ig = converged_is(max(round(rand(1) * numel(converged_is)), 1));
            x = ptr_sols.x(:, :, ig);
            u = ptr_sols.u(:, :, ig);
            p = ptr_sols.p(:, ig);
            
            i_filter = dV_filter(ig);
            ID1_i = ID1(comb_i.IDs_1_i_batch(i_filter));
            ID2_i = ID2(comb_i.IDs_2_i(i_filter));
            ...
            % Initial conditions
            t_plot = linspace(0, tf, 100) + t0;%(comb_i.IDs_1_i_batch(dV_filter(ig)));
            ...
            problem.x0 = [x_1_AA_filtered(:, ig); m0_filtered(ig)];
