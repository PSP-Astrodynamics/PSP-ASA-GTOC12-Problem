...
mf = m0 - alpha * u_max * (tf - t0) * t_star / m_star;
...
[x_kep_ast, x_cart_ast] = get_cartesian_states(ast, mu, tf);

% Earth -> Ast
x_1_EA = x_cart_ast(:, :, 1);
x_2_EA = repmat(x_cartesian_earth(tspan(2)), 1, numel(IDs));

% Earth -> Ast (up to one rev most likely)
[v1_best_EA, v2_best_EA, dV_best_EA, N_best] = best_lambert_thruN(x_1_EA, x_2_EA, (tspan(2) - tspan(1)) * ones([1, numel(IDs)]), 2, 0, 6 / v_star);
...
x_2_EA_filtered = x_2_EA(:, lambert_filter);
v1_best_EA_filtered = v1_best_EA(:, lambert_filter);
v2_best_EA_filtered = v2_best_EA(:, lambert_filter);
dV_best_EA_filtered = dV_best_EA(lambert_filter);

%% Create Guesses from Lambert
guesses = {};
for i = 1 : n_guesses
    guess_i = lambert_simple_initial_guess(x_1_EA_filtered(1:6, i), x_2_EA_filtered(:, i), v1_best_EA_filtered(:, i), v2_best_EA_filtered(:, i), N_best(lambert_filter(i)), t_k, m0);
    ...
    guesses.p(:, i) = 6 / v_star * v1_best_EA_filtered(:, i) / norm(v1_best_EA_filtered(:, i));
end
...
problem = DeterministicProblem([x_1_EA_filtered(1:6, 1); m0], x_2_EA_filtered(1:6, 1), N, u_hold, tf, f, guess_0, convex_constraints, min_fuel_objective, scale = scale, integration_tolerance = 1e-12, discretization_method = "error", N_sub = 1, Name = "Ast2Earth_fixed");
...
for i = 1 : ceil(n_guesses / batch_size)
    ...
    for j = 1 : batch_size
        ...
        problem_new = problem;
        problem_new.x0 = [x_1_EA_filtered(1:6, index); m0];
        problem_new.xf = x_2_EA_filtered(1:6, index);
        ...
        ptr_sol_index = ptr(problem_new, ptr_ops, parser, quiet = 0);
...
dV_ratio = dV_rocket_equation ./ dV_best_EA_filtered(converged_is)';

figure
scatter(dV_best_EA_filtered(converged_is) * v_star, dV_rocket_equation * v_star); hold on
...
dataset_1p0ToF_14yr_0p45m.dV_lambert = dV_best_EA_filtered(converged_is);
dataset_1p0ToF_14yr_0p45m.dV_lowthrust = dV_rocket_equation;
dataset_1p0ToF_14yr_0p45m.v1_ast = x_1_EA_filtered(4:6, converged_is);
dataset_1p0ToF_14yr_0p45m.v2_ast = x_2_EA_filtered(4:6, converged_is); 
dataset_1p0ToF_14yr_0p45m.v1_lambert = v1_best_EA_filtered(:, converged_is);
dataset_1p0ToF_14yr_0p45m.v2_lambert = v2_best_EA_filtered(:, converged_is);
...
save dataset_EA_1p3ToF_0yr.mat dataset_1p0ToF_14yr_0p45m
%%
...
x_0_opt = [x_1_EA_filtered(1:6, ig); m0];
...
for ic = 1 : numel(converged_is)
    ...
    % Initial conditions
    t_plot = linspace(0, tf, 100) + tspan(1);
    x_cartesian_earth_plot = zeros([6, numel(t_plot)]);
    x_cartesian_ast_plot = zeros([6, numel(t_plot)]);
    for k = 1 : numel(t_plot)
        x_cartesian_earth_plot(:, k) = x_cartesian_earth(t_plot(k));
    ...
    problem.x0 = [x_1_EA_filtered(1:6, ig); m0];
    problem.xf = x_2_EA_filtered(:, ig);
