...
dataset_filename = strrep(sprintf("dataset_AE_%.1fToF_%.1fyr_%.2fmf", ToF * t_star / year_to_sec, t0 * t_star / year_to_sec, mf_constraint), '.', 'p');
...
[v1_best_AE_pos, v2_best_AE_pos, dV_best_AE_pos, N_best_pos] = best_lambert_thruN(x_1_AE, x_2_AE, (tspan(2) - tspan(1)) * ones([1, numel(IDs)]), 2, 0, 6 / v_star, direction = ones([1, numel(IDs)]));
[v1_best_AE_neg, v2_best_AE_neg, dV_best_AE_neg, N_best_neg] = best_lambert_thruN(x_1_AE, x_2_AE, (tspan(2) - tspan(1)) * ones([1, numel(IDs)]), 2, 0, 6 / v_star, direction = -ones([1, numel(IDs)]));
[dV_best_AE, dV_best_i] = min([dV_best_AE_pos, dV_best_AE_neg], [], 2);

v1_best_AE = zeros([3, numel(IDs)]);
v2_best_AE = zeros([3, numel(IDs)]);
N_best = zeros([1, numel(IDs)]);
for i = 1 : numel(IDs)
    if dV_best_i(i) == 1
        v1_best_AE(:, i) = v1_best_AE_pos(:, i);
        v2_best_AE(:, i) = v2_best_AE_pos(:, i);
        N_best(i) = N_best_pos(i);
    elseif dV_best_i(i) == 2
        v1_best_AE(:, i) = v1_best_AE_neg(:, i);
        v2_best_AE(:, i) = v2_best_AE_neg(:, i);
...
max_dV = dV_max;
multiplier = 1 / 1000; % Some lambert solutions are hyperbolic so need to deal with those
lambert_filter = find(e < 1);%find(dV_best_AE < max_dV / multiplier);
...
problem = DeterministicProblem([x_1_AE_filtered(1:6, 1); m0], x_2_AE_filtered(1:6, 1), N, u_hold, tf, f, guess_0, convex_constraints, min_fuel_objective, scale = scale, integration_tolerance = 1e-12, discretization_method = "error", N_sub = 1, Name = "Ast2Earth_fixedmf");
...
%defect_histories = zeros(N + 1, n_guesses);
...
parfor i = 1 : ceil(n_guesses / batch_size)
    ...
    for j = 1 : batch_size
        ...
        problem_new = problem;
        problem_new.x0 = x_1_AE_filtered(1:6, index);
        problem_new.xf = [x_2_AE_filtered(1:6, index); mf_constraint];
        problem_new.initial_bc = @(x, p) [x(1:6) - problem_new.x0(1:6); 0];
        problem_new.terminal_bc = @(x, p, x_ref, p_ref) [x(1:3) - problem_new.xf(1:3); x(4:6) - p(1:3) - problem_new.xf(4:6); x(7) - problem_new.xf(7)];
        ...
        %defect_histories_batch{j} = reshape(sum(vecnorm(ptr_sol_index.Delta, 2, 1),2), [], 1);
...
dataset.m0 = reshape(ptr_sols.x(7, 1, converged_is), 1, []);
...
x_0_opt = [x_1_AE_filtered(1:6, ig); x(7, 1)];
...
%%
ast.Lf = wrapTo2Pi(ast.M0 + sqrt(ast.a .^ 3) * t0);

figure
scatter3(ast.a(converged_is(1:10:end)), ast.Lf(converged_is(1:10:end)), dataset.dV_lowthrust(1:10:end) * v_star)

%% Plot a bunch of solutions
[dV_sorted, dV_sort_i] = sort(dataset.m0, "ascend");

figure
for ic = 1000 : 1050%numel(converged_is)
    ...
    problem.x0 = [x_1_AE_filtered(1:6, ig); x(7, 1)];
