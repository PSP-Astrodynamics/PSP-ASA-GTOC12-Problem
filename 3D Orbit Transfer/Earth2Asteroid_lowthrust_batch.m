...
% Earth -> Ast
x_1_EA = repmat(x_cartesian_earth(tspan(1)), 1, numel(IDs));
x_2_EA = x_cart_ast(:, :, 1);
...
problem = DeterministicProblem([x_1_EA_filtered(1:6, 1); m0], x_2_EA_filtered(1:6, 1), N, u_hold, tf, f, guess_0, convex_constraints, min_fuel_objective, scale = scale, integration_tolerance = 1e-12, discretization_method = "error", N_sub = 1, Name = "Earth2Ast_fixed");
...
parfor i = 1 : ceil(n_guesses / batch_size)
    ...
    for j = 1 : batch_size
        ...
        problem_new.initial_bc = @(x, p) [x(1:3) - problem_new.x0(1:3); x(4:6) - p(1:3) - problem_new.x0(4:6); x(7) - problem_new.x0(7)];
...
x_0_opt = [x_1_EA_filtered(1:6, ig); m0] + [0; 0; 0; p(1:3); 0];
