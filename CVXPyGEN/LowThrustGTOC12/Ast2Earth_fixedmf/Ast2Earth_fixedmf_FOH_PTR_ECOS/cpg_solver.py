...
from Ast2Earth_fixedmf_FOH_PTR_ECOS import cpg_module
...
def cpg_solve(prob, updated_params=None, **kwargs):
    ...
    prob.var_dict['v_0'].save_value(np.array(res.cpg_prim.v_0).reshape((6, 1), order='F'))
    prob.var_dict['v_N'].save_value(np.array(res.cpg_prim.v_N).reshape((7, 1), order='F'))
    ...
    prob.constraints[17].save_dual_value(np.array(res.cpg_dual.d17).reshape((6, 6), order='F'))
    prob.constraints[18].save_dual_value(np.array(res.cpg_dual.d18).reshape((3, 3), order='F'))
    prob.constraints[19].save_dual_value(np.array(res.cpg_dual.d19).reshape((3, 3), order='F'))
    prob.constraints[20].save_dual_value(np.array(res.cpg_dual.d20).reshape(1))
    prob.constraints[21].save_dual_value(np.array(res.cpg_dual.d21).reshape((1, 15), order='F'))
...
def cpg_gradient(prob, gradient_sol_primal=None, gradient_sol_dual=None):
    ...
    if gradient_sol_primal is not None and gradient_sol_dual is not None:
        ...
        gradient_sol.primal = [0] * 438
        gradient_sol.dual = [0] * 753
        use_sol = False
    ...
    prob.param_dict['x_0'].gradient = np.array(pdelta.x_0).reshape((6, 1), order='F')
    prob.param_dict['x_f'].gradient = np.array(pdelta.x_f).reshape((7, 1), order='F')
