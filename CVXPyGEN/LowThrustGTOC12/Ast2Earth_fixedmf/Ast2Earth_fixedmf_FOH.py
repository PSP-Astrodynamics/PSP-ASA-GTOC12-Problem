...
if not load:
    ...
    loaded_data = np.load(r'C:\Users\thatf\OneDrive\Documents\ASA\PSP-ASA-GTOC12-Problem\CVXPyGEN\LowThrustGTOC12\Ast2Earth_fixedmf\saved_Ast2Earth_fixedmf_input_FOH.npz', allow_pickle=True)
    ...
    # Saved Problem
    dbfile = open(r'C:\Users\thatf\OneDrive\Documents\ASA\PSP-ASA-GTOC12-Problem\CVXPyGEN\LowThrustGTOC12\Ast2Earth_fixedmf\Ast2Earth_fixedmf_prob_FOH', 'rb')    
...
if str(type(problem)) == "<class 'array.array'>" or str(type(problem)) == "<class 'numpy.ndarray'>":
    ...
    v_0 = cp.Variable((nx - 1, 1), name = 'v_0')
    ...
    x_0_param = cp.Parameter((nx - 1, 1), name = 'x_0')
    ...
    constraints = dynamics_constraints + [ # Dynamics constraint
                   ...
                   cp.norm2(p) <= v_max_nd, # Arrival velocity constraint
                   X[0:6, 0] + v_0 == x_0_param.flatten(), # Initial condition constraint
                   X[0:3, N] + v_N[0:3] == x_f_param[0:3].flatten(), X[3:6, N] + v_N[3:6] == p.flatten() + x_f_param[3:6].flatten(), X[6, N] + v_N[6] == x_f_param[6], # Terminal condition constraint
...
if save:
    np.savez(r'C:\Users\thatf\OneDrive\Documents\ASA\PSP-ASA-GTOC12-Problem\CVXPyGEN\LowThrustGTOC12\Ast2Earth_fixedmf\saved_Ast2Earth_fixedmf_input_FOH.npz', A_k = A_k, B_k_minus = B_k_minus, B_k_plus = B_k_plus, c_k = c_k, w_vc = w_vc, w_tr = w_tr, delta_t = delta_t, N = N, x_0 = x_0, x_f = x_f, x_ref = x_ref, u_ref = u_ref)
    
    dbfile = open(r'C:\Users\thatf\OneDrive\Documents\ASA\PSP-ASA-GTOC12-Problem\CVXPyGEN\LowThrustGTOC12\Ast2Earth_fixedmf\Ast2Earth_fixedmf_prob_FOH', 'ab')
...
cpg.generate_code(problem, code_dir='Ast2Earth_fixedmf_FOH_PTR_ECOS', solver = "ECOS")
...
## 
# ## 
# from Ast2Earth_fixedmf_FOH_PTR_ECOS.cpg_solver import cpg_solve
# ## 
