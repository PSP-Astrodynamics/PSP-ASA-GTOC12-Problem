...
from Ast2Ast_fixed_FOH_PTR_ECOS import cpg_module
...
def cpg_solve(prob, updated_params=None, **kwargs):
    ...
    prob.constraints[16].save_dual_value(np.array(res.cpg_dual.d16).reshape((6, 6), order='F'))
    prob.constraints[17].save_dual_value(np.array(res.cpg_dual.d17).reshape((7, 7), order='F'))
    prob.constraints[18].save_dual_value(np.array(res.cpg_dual.d18).reshape((1, 15), order='F'))
...
def cpg_gradient(prob, gradient_sol_primal=None, gradient_sol_dual=None):
    ...
    if gradient_sol_primal is not None and gradient_sol_dual is not None:
        ...
        gradient_sol.primal = [0] * 434
        gradient_sol.dual = [0] * 778
        use_sol = False
